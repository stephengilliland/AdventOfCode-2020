...
currRules = []
solvedRules = []
rulesUnresolved = True
ruleContents = []
ruleNum = ''
ruleHasDig = False
for x in range(len(rules)):
    currRules = rules[x].split(" ")
    if 'a' in currRules:
        currARule = x
    if 'b' in currRules:
        currBRule = x
...
while rulesUnresolved:

    for x in range(len(rules)):

        currRule = rules[x].split(' ')
        print("")
        print("Bare Contents",rules[x][rules[x].find(':')+2:])
        ruleContents = rules[x][rules[x].find(':')+2:].split(' ')
        ruleNum = rules[x][:rules[x].find(':')]
        print("Rule Number",ruleNum)

        for y in range(len(ruleContents)):
            if ruleContents[y] in rulesDict:
                if rulesDict[ruleContents[y]] == 'a' or rulesDict[ruleContents[y]] == 'b':
                    ruleContents[y] = rulesDict[ruleContents[y]]
                else:
                    ruleContents[y] = '[' + rulesDict[ruleContents[y]] + ']'

        ruleHasDigit = False
        for y in range(len(ruleContents)):
            if ruleContents[y].isdigit():
                ruleHasDigit = True
        
        if not ruleHasDigit:
            print("Adding:",ruleNum)
            #print(ruleContents)
            rulesDict[ruleNum] = ''.join(ruleContents)

        rules[x] = ruleNum + ':' + ' ' + ' '.join(ruleContents)
        print("Rule:",rules[x])
        print("Contains",ruleContents)
    #print(rulesDict)
    if len(rulesDict) == len(rules):
        print('')
        break
zeroRule = rulesDict['0']
print(rulesDict['0'])
quit()


possibs = []
possibs.append(zeroRule)
newPossibs = []
newPossibs.append(zeroRule)
parenFound = True
startParen = 0
endParen = 0
startFound = False
...
for d in range(100000): 
    ...
    if not d % 1:
        print("Run",d)
        print(len(newPossibs),"Possibilities")
        print(len(newPossibs[0]))
        print(newPossibs[0])
        seen.clear()
        seen = set()
    #if len(possibs) == 524288
    #    break
    if len(newPossibs) == 0:
        print("No New")
        break
    possibs = copy.deepcopy(newPossibs)
    newPossibs = []
    while len(possibs):
        zeroRule = possibs[0]
        del possibs[0]
        #print(zeroRule)
        if zeroRule not in seen:
            #print(possibs)
            parenFound = True
            while parenFound:
                parenFound = False
                for a in range(len(zeroRule)):
                    if zeroRule[a] == '[' and not startFound:
                        startParen = a
                        startFound = True
                    elif zeroRule[a] == '[' and startFound:
                        startParen = a
                    if zeroRule[a] == ']' and startFound:
                        endParen = a
                        endFound = True
                    if startParen and endParen:
                        substring = zeroRule[startParen+1:endParen]
                        #print("Substring:",substring)
                        pipeLoc = substring.find('|')
                        #(firstCombo, _, secCombo) = substring.partition('|')
                        if '|' in substring:
                            firstCombo = substring[:pipeLoc]
                            secCombo = substring[pipeLoc+1:]
                            newRule = list(zeroRule)
                            for z in range(startParen, endParen+1):
                                newRule.pop(startParen)
                            newRule.insert(startParen, firstCombo)
                            newOne = ''.join(newRule)
                            newRule = list(zeroRule)
                            for z in range(startParen, endParen+1):
                                newRule.pop(startParen)
                            newRule.insert(startParen, secCombo)
                            newTwo = ''.join(newRule)
                            if newOne not in seen:
                                newPossibs.append(newOne)
                            if newTwo not in seen:
                                newPossibs.append(newTwo)
                        else:
                            zeroRule = list(zeroRule)
                            for z in range(startParen, endParen+1):
                                zeroRule.pop(startParen)
                            zeroRule.insert(startParen, substring)
                            zeroRule = ''.join(zeroRule)
                            if zeroRule not in seen:
                                newPossibs.append(zeroRule)
                        startFound = False
                        endFound = False
...
for x in range(len(possibs)):
    if '[' in possibs[x]:
        ...
        #print(possibs[x])
        goodPoss.append(possibs[x])
ctr = 0
for x in range(len(inpt)):
    if inpt[x] in goodPoss:
        ctr += 1
print("answer =",ctr)
#print(rulesDict)

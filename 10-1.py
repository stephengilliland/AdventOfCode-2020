adpt = [1 , 2 , 3 , 4 , 7 , 8 , 9 , 10 , 13 , 16 , 17 , 18 , 19 , 22 , 23 , 24 , 25 , 26 , 29 , 30 , 31 , 32 , 33 , 36 , 37 , 38 , 41 , 42 , 43 , 44 , 47 , 50 , 51 , 52 , 53 , 56 , 57 , 58 , 61 , 64 , 65 , 66 , 69 , 70 , 71 , 72 , 75 , 76 , 77 , 80 , 81 , 82 , 83 , 84 , 87 , 88 , 89 , 90 , 93 , 96 , 97 , 98 , 99 , 100 , 103 , 104 , 105 , 108 , 111 , 114 , 115 , 118 , 121 , 122 , 123 , 124 , 127 , 128 , 129 , 130 , 131 , 134 , 137 , 138 , 139 , 140 , 143 , 146 , 149 , 150 , 151 , 152 , 155 , 156 , 157 , 160 , 161 , 164 , 165 , 166 , 167 , 168 , 171 , 172 , 173 , 174 , 177 , 180 , 181 , 182 , 183 , 184]

jolts = 2812
adpt.sort()
onesCtr = 1
threesCtr = 1
x = 0
answer = 0
ansCtr = 0
print(len(adpt))

while x <= len(adpt)-2:
    if adpt[x+1] == adpt[x] + 1:
        onesCtr += 1
        x += 1
    elif adpt[x+1] == adpt[x] + 2:
        x += 1
    elif adpt[x+1] == adpt[x] + 3:
        threesCtr += 1
        x += 1
    else:
        break

answer = onesCtr*threesCtr
print(answer)
    



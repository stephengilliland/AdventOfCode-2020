...
for x in range(len(foods)):
    currMeal = foods[x]
    currAllergs = currMeal[currMeal.find(":")+1:].split(",")
    currMeal = currMeal[:currMeal.find(':')].split(' ')
    #print(currMeal)
    for y in range(len(currAllergs)):
        currAllerg = currAllergs[y]
        if currAllerg not in seenAllergs:
            seenAllergs.append(currAllerg)
            print(currAllerg)

        #print(currAllerg)
        for z in range(len(currMeal)):
            currIng = currMeal[z]

            currAllergGood = True
            for a in range(len(foods)):
                compMeal = foods[a]
                compareAllergs = compMeal[compMeal.find(":")+1:].split(",")
                compareMeal = compMeal[:compMeal.find(':')].split(' ')

                if (currAllerg not in compareAllergs):
                    pass
                else:
                    if currAllerg == 'dairy':
                        dairy.append(compareMeal)
                    if currAllerg == 'nuts':
                        nuts.append(compareMeal)
                    if currAllerg == 'soy':
                        soy.append(compareMeal)
                    if currAllerg == 'peanuts':
                        peanuts.append(compareMeal)
                    if currAllerg == 'fish':
                        fish.append(compareMeal)
                    if currAllerg == 'eggs':
                        eggs.append(compareMeal)
                    if currAllerg == 'sesame':
                        sesame.append(compareMeal)
                    if currAllerg == 'shellfish':
                        shellfish.append(compareMeal)

                    currAllergGood = False
            if currAllergGood:
                allergDict[currAllerg] = currIng
goodIng = True
allergeries = set()
remove = set()
print("DAIRY")
...
for x in range(len(foods)):
    ...
    currAllergs = currMeal[currMeal.find(":")+1:].split(",")
    currMeal = currMeal[:currMeal.find(':')].split(' ')
    if 'dairy' not in currAllergs:
        for al in allergeries:
            if al in currMeal:
                remove.add(al)
print(allergeries)
allergeries.clear()
...
allergeries.clear()
print("PEANUTS")
for ing in peanuts[0]:
    goodIng = True
    for x in range(1, len( peanuts)):
        if ing in peanuts[x]:
            pass
        else:
            goodIng = False
...
print("SHELLFISH")
for ing in shellfish[0]:
    goodIng = True
    for x in range(1, len(shellfish)):
        if ing in shellfish[x]:
            pass
        else:
            goodIng = False
    if goodIng:
...
for x in range(len(foods)):
    ...
    currAllergs = currMeal[currMeal.find(":")+1:].split(",")
    currMeal = currMeal[:currMeal.find(':')].split(' ')
    if 'shellfish' not in currAllergs:
        for al in allergeries:
            if al in currMeal:
                remove.add(al)
print(allergeries)
...
ctr = 0
for x in range(len(foods)):
    currFood = foods[x][:foods[x].find(':')].split(' ')
    for y in range(len(currFood)):
        if currFood[y] not in allergeries:
            ctr += 1
print(ctr)
bads = []
for al in allergeries:
